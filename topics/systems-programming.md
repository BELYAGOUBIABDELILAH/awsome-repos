# Systems Programming Repositories

*Operating systems, compilers, and low-level programming*

---

## 📊 Overview

- **Total Repositories**: 1941
- **First Seen**: 2026-07-30
- **Last Updated**: 2026-08-02
- **Average Stars**: 17828

---

## 🔥 Trending Today

| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [rust-lang/rust](../repos/rust-lang/rust.md) | ⭐ 117,447 | Rust |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [shadcn-ui/ui](../repos/shadcn-ui/ui.md) | ⭐ 123,032 | TypeScript |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [NousResearch/hermes-agent](../repos/NousResearch/hermes-agent.md) | ⭐ 241,422 | Python |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [ChromeDevTools/chrome-devtools-mcp](../repos/ChromeDevTools/chrome-devtools-mcp.md) | ⭐ 50,924 | TypeScript |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [QuantumNous/new-api](../repos/QuantumNous/new-api.md) | ⭐ 47,310 | Go |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [aquasecurity/trivy](../repos/aquasecurity/trivy.md) | ⭐ 37,782 | Go |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [Tencent/WeKnora](../repos/Tencent/WeKnora.md) | ⭐ 21,340 | Go |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [nageoffer/ragent](../repos/nageoffer/ragent.md) | ⭐ 3,891 | Java |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [protocolbuffers/protobuf](../repos/protocolbuffers/protobuf.md) | ⭐ 71,999 | C++ |  |
| Repository | Stars | Language | Last Trending |
|------------|-------|----------|---------------|
| [google/benchmark](../repos/google/benchmark.md) | ⭐ 10,385 | C++ |  |


---

## ⭐ Most Starred

| Repository | Stars | Language | First Seen | Trending Days |
|------------|-------|----------|------------|---------------|
| [codecrafters-io/build-your-own-x](../repos/codecrafters-io/build-your-own-x.md) | ⭐ 534,798 | Markdown | 2026-08-02 | 1 |
| [public-apis/public-apis](../repos/public-apis/public-apis.md) | ⭐ 464,529 | Python | 2026-08-01 | 5 |
| [donnemartin/system-design-primer](../repos/donnemartin/system-design-primer.md) | ⭐ 362,245 | Python | 2026-08-03 | 5 |
| [practical-tutorials/project-based-learning](../repos/practical-tutorials/project-based-learning.md) | ⭐ 278,836 | Python | 2026-08-08 | 3 |
| [react/react](../repos/react/react.md) | ⭐ 247,637 | JavaScript | 2026-08-04 | 2 |
| [NousResearch/hermes-agent](../repos/NousResearch/hermes-agent.md) | ⭐ 241,422 | Python | 2026-08-01 | 14 |
| [facebook/react](../repos/facebook/react.md) | ⭐ 219,500 | JavaScript | 2026-07-30 | 3 |
| [vuejs/vue](../repos/vuejs/vue.md) | ⭐ 210,192 | TypeScript | 2026-08-04 | 1 |
| [ossu/computer-science](../repos/ossu/computer-science.md) | ⭐ 208,714 | HTML | 2026-08-03 | 7 |
| [n8n-io/n8n](../repos/n8n-io/n8n.md) | ⭐ 202,134 | TypeScript | 2026-08-11 | 3 |

---

## 📈 Trending This Week

| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [rust-lang/rust](../repos/rust-lang/rust.md) | ⭐ 117,447 | Rust | 1 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [astral-sh/uv](../repos/astral-sh/uv.md) | ⭐ 89,217 | Rust | 1 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [shadcn-ui/ui](../repos/shadcn-ui/ui.md) | ⭐ 123,032 | TypeScript | 1 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [harry0703/MoneyPrinterTurbo](../repos/harry0703/MoneyPrinterTurbo.md) | ⭐ 118,456 | Python | 1 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [NousResearch/hermes-agent](../repos/NousResearch/hermes-agent.md) | ⭐ 241,422 | Python | 3 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [node-red/node-red](../repos/node-red/node-red.md) | ⭐ 23,614 | JavaScript | 1 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [ChromeDevTools/chrome-devtools-mcp](../repos/ChromeDevTools/chrome-devtools-mcp.md) | ⭐ 50,924 | TypeScript | 3 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [paritytech/polkadot-sdk](../repos/paritytech/polkadot-sdk.md) | ⭐ 2,799 | Rust | 1 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [t8y2/dbx](../repos/t8y2/dbx.md) | ⭐ 17,401 | Rust | 2 |
| Repository | Stars | Language | Days Trending |
|------------|-------|----------|---------------|
| [QuantumNous/new-api](../repos/QuantumNous/new-api.md) | ⭐ 47,310 | Go | 1 |

---

## 🆕 New This Month

| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [langchain-ai/langchain](../repos/langchain-ai/langchain.md) | ⭐ 75,200 | Python | 2026-08-10 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [Significant-Gravitas/AutoGPT](../repos/Significant-Gravitas/AutoGPT.md) | ⭐ 186,850 | Python | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [frappe/hrms](../repos/frappe/hrms.md) | ⭐ 8,515 | Python | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [ComposioHQ/awesome-claude-skills](../repos/ComposioHQ/awesome-claude-skills.md) | ⭐ 73,881 | Python | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [langchain-ai/open-swe](../repos/langchain-ai/open-swe.md) | ⭐ 10,496 | Python | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [Unclecheng-li/VulnClaw](../repos/Unclecheng-li/VulnClaw.md) | ⭐ 2,600 | Python | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [Snailclimb/JavaGuide](../repos/Snailclimb/JavaGuide.md) | ⭐ 157,730 | JavaScript | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [microsoft/Web-Dev-For-Beginners](../repos/microsoft/Web-Dev-For-Beginners.md) | ⭐ 96,607 | JavaScript | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [drawdb-io/drawdb](../repos/drawdb-io/drawdb.md) | ⭐ 39,289 | JavaScript | 2026-08-06 |
| Repository | Stars | Language | First Seen |
|------------|-------|----------|------------|
| [react/create-react-app](../repos/react/create-react-app.md) | ⭐ 103,296 | JavaScript | 2026-08-06 |

---

## 📅 Recent History

### [2026-09-04](../archive/2026/09/2026-09-04.md)
- 242 repositories trending
- Top: NousResearch&#x2F;hermes-agent

### [2026-09-03](../archive/2026/09/2026-09-03.md)
- 205 repositories trending
- Top: NousResearch&#x2F;hermes-agent

### [2026-09-02](../archive/2026/09/2026-09-02.md)
- 266 repositories trending
- Top: NousResearch&#x2F;hermes-agent

### [2026-09-01](../archive/2026/09/2026-09-01.md)
- 257 repositories trending
- Top: flutter&#x2F;flutter

### [2026-08-29](../archive/2026/08/2026-08-29.md)
- 258 repositories trending
- Top: microsoft&#x2F;PowerToys


---

## 🏆 Hall of Fame

*Most frequently trending Systems Programming repositories*

| Repository | Trending Count | First Seen | Latest |
|------------|----------------|------------|--------|
| [apache/spark](../repos/apache/spark.md) | 31 days | 2026-08-01 | 2026-09-04 |
| [logseq/logseq](../repos/logseq/logseq.md) | 30 days | 2026-08-01 | 2026-09-04 |
| [E3SM-Project/E3SM](../repos/E3SM-Project/E3SM.md) | 30 days | 2026-08-01 | 2026-09-04 |
| [OpenZeppelin/openzeppelin-contracts](../repos/OpenZeppelin/openzeppelin-contracts.md) | 30 days | 2026-08-01 | 2026-09-04 |
| [The-OpenROAD-Project/OpenROAD](../repos/The-OpenROAD-Project/OpenROAD.md) | 30 days | 2026-08-01 | 2026-09-04 |
| [metabase/metabase](../repos/metabase/metabase.md) | 29 days | 2026-08-01 | 2026-09-04 |
| [semgrep/semgrep](../repos/semgrep/semgrep.md) | 29 days | 2026-08-01 | 2026-09-04 |
| [analogdevicesinc/hdl](../repos/analogdevicesinc/hdl.md) | 29 days | 2026-08-01 | 2026-09-04 |
| [koalaman/shellcheck](../repos/koalaman/shellcheck.md) | 28 days | 2026-08-01 | 2026-09-04 |
| [vlang/v](../repos/vlang/v.md) | 28 days | 2026-08-01 | 2026-09-04 |

---

## 📊 Statistics

### Language Breakdown
- **Go**: 178 repositories (9%)
- **C++**: 138 repositories (7%)
- **Python**: 129 repositories (7%)
- **Rust**: 126 repositories (6%)
- **TypeScript**: 124 repositories (6%)

### Trending Frequency
- **Daily Average**: 246.1 repositories
- **Peak Day**: 2026-08-19 (382 repositories)
- **Growth Rate**: 57% this month

---

## 🔗 Related Topics


---

## 🔍 Browse All Repositories

- [tensorflow/tensorflow](../repos/tensorflow/tensorflow.md) - An Open Source Machine Learning Framework for Everyone
- [pytorch/pytorch](../repos/pytorch/pytorch.md) - Tensors and Dynamic neural networks in Python with strong GPU acceleration
- [facebook/react](../repos/facebook/react.md) - The library for web and native user interfaces
- [microsoft/vscode](../repos/microsoft/vscode.md) - Visual Studio Code
- [rust-lang/rust](../repos/rust-lang/rust.md) - Empowering everyone to build reliable and efficient software.
- [vercel/next.js](../repos/vercel/next.js.md) - The React Framework
- [tauri-apps/tauri](../repos/tauri-apps/tauri.md) - Build smaller, faster, and more secure desktop applications with a web frontend
- [denoland/deno](../repos/denoland/deno.md) - A modern runtime for JavaScript and TypeScript.
- [huggingface/transformers](../repos/huggingface/transformers.md) - 🤗 Transformers: the model-definition framework for state-of-the-art machine learning models in t...
- [astral-sh/uv](../repos/astral-sh/uv.md) - An extremely fast Python package and project manager, written in Rust.
- [shadcn-ui/ui](../repos/shadcn-ui/ui.md) - A set of beautifully-designed, accessible components and a code distribution platform. Works with...
- [anthropics/anthropic-sdk-python](../repos/anthropics/anthropic-sdk-python.md) - SDK for Anthropic&#39;s Claude AI
- [excalidraw/excalidraw](../repos/excalidraw/excalidraw.md) - Virtual whiteboard for sketching hand-drawn like diagrams
- [langchain-ai/langchain](../repos/langchain-ai/langchain.md) - Building applications with LLMs through composability
- [mvanhorn/last30days-skill](../repos/mvanhorn/last30days-skill.md) - AI agent skill that researches any topic across Reddit, X, YouTube, HN, Polymarket, and the web -...
- [paperswithbacktest/awesome-systematic-trading](../repos/paperswithbacktest/awesome-systematic-trading.md) - A curated list of awesome libraries, packages, strategies, books, blogs, tutorials for systematic...
- [microsoft/AI-For-Beginners](../repos/microsoft/AI-For-Beginners.md) - 12 Weeks, 24 Lessons, AI for All!
- [geo-tp/ESP32-Bit-Pirate](../repos/geo-tp/ESP32-Bit-Pirate.md) - A Hardware Hacking Tool with Web-Based CLI That Speaks Every Protocol 
- [1jehuang/jcode](../repos/1jehuang/jcode.md) - The most RAM efficient harness
- [huggingface/speech-to-speech](../repos/huggingface/speech-to-speech.md) - Build local voice agents with open-source models
- [harry0703/MoneyPrinterTurbo](../repos/harry0703/MoneyPrinterTurbo.md) - 利用 AI 大模型和自动化工作流，根据主题或关键词一键生成高清短视频。Generate HD short videos from a topic or keyword with an autom...
- [public-apis/public-apis](../repos/public-apis/public-apis.md) - A collective list of free APIs
- [NousResearch/hermes-agent](../repos/NousResearch/hermes-agent.md) - The agent that grows with you
- [0x4m4/hexstrike-ai](../repos/0x4m4/hexstrike-ai.md) - HexStrike AI MCP Agents is an advanced MCP server that lets AI agents (Claude, GPT, Copilot, etc....
- [Panniantong/Agent-Reach](../repos/Panniantong/Agent-Reach.md) - Give your AI agent eyes to see the entire internet. Read &amp; search Twitter, Reddit, YouTube, GitHu...
- [langchain-ai/deepagents](../repos/langchain-ai/deepagents.md) - The batteries-included agent harness.
- [WhiskeySockets/Baileys](../repos/WhiskeySockets/Baileys.md) - Socket-based TS&#x2F;JavaScript API for WhatsApp Web
- [huggingface/transformers.js](../repos/huggingface/transformers.js.md) - State-of-the-art Machine Learning for the web. Run 🤗 Transformers directly in your browser, with...
- [node-red/node-red](../repos/node-red/node-red.md) - Low-code programming for event-driven applications
- [freestylefly/awesome-gpt-image-2](../repos/freestylefly/awesome-gpt-image-2.md) - Prompt as Code | GPT-Image2 工业级提示词引擎与模板库，530+ 个案例逆向工程，20+ 套工业级模板，并提炼出Skills，持续更新中
- [nyxxbit/discord-quest-completer](../repos/nyxxbit/discord-quest-completer.md) - Auto-complete every Discord Quest in seconds. Paste one script, get all rewards. Resilient module...
- [byoungd/up](../repos/byoungd/up.md) - An advanced guide which might benefit you a lot 🎉 . 韩先凯的人生进阶指南 离谱的人生&#x2F;人生进阶 离谱的英语学习指南&#x2F;英语学习教程&#x2F;英语学习&#x2F;学英语
- [TencentCloud/TencentDB-Agent-Memory](../repos/TencentCloud/TencentDB-Agent-Memory.md) - TencentDB Agent Memory is a team-level memory hub for AI Agents — turning conversations, docs, an...
- [moeru-ai/airi](../repos/moeru-ai/airi.md) - 💖🧸 Self hosted, you-owned Grok Companion, a container of souls of waifu, cyber livings to bring...
- [modelcontextprotocol/ext-apps](../repos/modelcontextprotocol/ext-apps.md) - Official repo for spec &amp; SDK of MCP Apps protocol - standard for UIs embedded AI chatbots, served...
- [ChromeDevTools/chrome-devtools-mcp](../repos/ChromeDevTools/chrome-devtools-mcp.md) - Chrome DevTools for coding agents
- [fingerprintjs/fingerprintjs](../repos/fingerprintjs/fingerprintjs.md) - The most advanced free and open-source browser fingerprinting library
- [continuedev/continue](../repos/continuedev/continue.md) - open-source coding agent
- [portainer/portainer](../repos/portainer/portainer.md) - Making Docker and Kubernetes management easy.
- [appwrite/appwrite](../repos/appwrite/appwrite.md) - Appwrite® - complete cloud infrastructure for your web, mobile and AI apps. Including Auth, Datab...
- [podman-desktop/podman-desktop](../repos/podman-desktop/podman-desktop.md) - Podman Desktop is the best free and open source tool to work with Containers and Kubernetes for d...
- [longbridge/gpui-component](../repos/longbridge/gpui-component.md) - Rust GUI components for building fantastic cross-platform desktop application by using GPUI.
- [paritytech/polkadot-sdk](../repos/paritytech/polkadot-sdk.md) - The Parity Polkadot Blockchain SDK
- [bruits/satteri](../repos/bruits/satteri.md) - High-performance Markdown and MDX processing for the JavaScript ecosystem
- [tursodatabase/turso](../repos/tursodatabase/turso.md) - A SQL database in Rust: SQLite-compatible, now also speaking Postgres (experimental). The LLVM of...
- [git-ai-project/git-ai](../repos/git-ai-project/git-ai.md) - A Git extension for tracking the AI-generated code in your repos
- [nolabs-ai/nono](../repos/nolabs-ai/nono.md) - Sandbox any AI agent in seconds - zero setup, zero latency.
- [agentgateway/agentgateway](../repos/agentgateway/agentgateway.md) - Next Generation Agentic Proxy for AI Agents and MCP servers
- [t8y2/dbx](../repos/t8y2/dbx.md) - 20 MB lightweight cross-platform database client for 90+ databases, including MySQL, PostgreSQL, ...
- [cocoindex-io/cocoindex](../repos/cocoindex-io/cocoindex.md) - Incremental engine for long horizon agents 🌟 Star if you like it!
- [googleworkspace/cli](../repos/googleworkspace/cli.md) - Google Workspace CLI — one command-line tool for Drive, Gmail, Calendar, Sheets, Docs, Chat, Admi...
- [cube-js/cube](../repos/cube-js/cube.md) - 📊 Cube Core is open-source semantic layer for AI, BI and embedded analytics
- [superradcompany/microsandbox](../repos/superradcompany/microsandbox.md) - 🧱 easy fast local-first microVM runtime and library
- [QuantumNous/new-api](../repos/QuantumNous/new-api.md) - A unified AI model hub for aggregation &amp; distribution. It supports cross-converting various LLMs ...
- [adithyan-ak/AgentHound](../repos/adithyan-ak/AgentHound.md) - Offensive security framework for AI agent infrastructure - recon, credential looting, model exfil...
- [github/gh-stack](../repos/github/gh-stack.md) - GitHub Stacked PRs
- [Ed1s0nZ/CyberStrikeAI](../repos/Ed1s0nZ/CyberStrikeAI.md) - The system of action for AI-native cybersecurity—where intent becomes governed execution, evidenc...
- [heroiclabs/nakama](../repos/heroiclabs/nakama.md) - Scalable open-source game backend server: multiplayer, matchmaking, leaderboards, chat, and socia...
- [aquasecurity/trivy](../repos/aquasecurity/trivy.md) - Find vulnerabilities, misconfigurations, secrets, SBOM in containers, Kubernetes, code repositori...
- [Tencent/WeKnora](../repos/Tencent/WeKnora.md) - Open-source LLM knowledge platform: turn raw documents into a queryable RAG, an autonomous reason...
- [nianzhibai/91](../repos/nianzhibai/91.md) - nine one
- [ccfos/nightingale](../repos/ccfos/nightingale.md) - Nightingale is to monitoring and alerting what Grafana is to visualization.
- [cert-manager/cert-manager](../repos/cert-manager/cert-manager.md) - Automatically provision and manage TLS certificates in Kubernetes
- [IceWhaleTech/CasaOS](../repos/IceWhaleTech/CasaOS.md) - CasaOS - A simple, easy-to-use, elegant open-source Personal Cloud system.
- [kserve/kserve](../repos/kserve/kserve.md) - Standardized Distributed Generative and Predictive AI Inference Platform for Scalable, Multi-Fram...
- [looplj/axonhub](../repos/looplj/axonhub.md) - ⚡️ Open-source AI Gateway — Use any SDK to call 100+ LLMs. Built-in failover, load balancing, cos...
- [crimera/piko](../repos/crimera/piko.md) - morphe patches for twitter and instagram
- [alibaba/fastjson2](../repos/alibaba/fastjson2.md) - 🚄 FASTJSON2 is a Java JSON  library with excellent performance.
- [DependencyTrack/dependency-track](../repos/DependencyTrack/dependency-track.md) - Dependency-Track is an intelligent Component Analysis platform that allows organizations to ident...
- [spring-projects/spring-ai](../repos/spring-projects/spring-ai.md) - An Application Framework for AI Engineering
- [nageoffer/ragent](../repos/nageoffer/ragent.md) - 企业级 Agentic RAG 智能体 - 全链路覆盖文档解析、多路检索、意图识别、问题重写、会话记忆、MCP 工具调用与深度思考。面向真实业务场景，从 0 到 1 完整工程实现。
- [grpc/grpc-java](../repos/grpc/grpc-java.md) - The Java gRPC implementation. HTTP&#x2F;2 based RPC
- [opensearch-project/OpenSearch](../repos/opensearch-project/OpenSearch.md) - 🔎 Open source distributed and RESTful search engine.
- [xoureldeen/Vectras-VM-Android](../repos/xoureldeen/Vectras-VM-Android.md) - It&#39;s a Virtual Machine App for Android Which is Based on QEMU
- [modelcontextprotocol/java-sdk](../repos/modelcontextprotocol/java-sdk.md) - The official Java SDK for Model Context Protocol servers and clients. Maintained in collaboration...
- [apache/maven](../repos/apache/maven.md) - Apache Maven core
- [debezium/debezium](../repos/debezium/debezium.md) - Change data capture for a variety of databases. Please log issues at https:&#x2F;&#x2F;github.com&#x2F;debezium&#x2F;...
- [OpenAPITools/openapi-generator](../repos/OpenAPITools/openapi-generator.md) - OpenAPI Generator allows generation of API client libraries (SDK generation), server stubs, docum...
- [apache/tika](../repos/apache/tika.md) - The Apache Tika toolkit detects and extracts metadata and text from over a thousand different fil...
- [protocolbuffers/protobuf](../repos/protocolbuffers/protobuf.md) - Protocol Buffers - Google&#39;s data interchange format
- [godotengine/godot](../repos/godotengine/godot.md) - Godot Engine – Multi-platform 2D and 3D game engine
- [gem5/gem5](../repos/gem5/gem5.md) - The official repository for the gem5 computer-system architecture simulator.
- [gabime/spdlog](../repos/gabime/spdlog.md) - Fast C++ logging library.
- [jrouwe/JoltPhysics](../repos/jrouwe/JoltPhysics.md) - A multi core friendly rigid body physics and collision detection library. Written in C++. Suitabl...
- [CLIUtils/CLI11](../repos/CLIUtils/CLI11.md) - CLI11 is a command line parser for C++11 and beyond that provides a rich feature set with a simpl...
- [google/benchmark](../repos/google/benchmark.md) - A microbenchmark support library
- [assimp/assimp](../repos/assimp/assimp.md) - The official Open-Asset-Importer-Library Repository. Loads 40+ 3D-file-formats into one unified a...
- [v8/v8](../repos/v8/v8.md) - The official mirror of the V8 Git repository
- [electron/electron](../repos/electron/electron.md) - :electron: Build cross-platform desktop apps with JavaScript, HTML, and CSS
- [microsoft/PowerToys](../repos/microsoft/PowerToys.md) - Microsoft PowerToys is a collection of utilities that supercharge productivity and customization ...
- [libuv/libuv](../repos/libuv/libuv.md) - Cross-platform asynchronous I&#x2F;O
- [DarkFlippers/unleashed-firmware](../repos/DarkFlippers/unleashed-firmware.md) - Flipper Zero Unleashed Firmware
- [fluent/fluent-bit](../repos/fluent/fluent-bit.md) - Fast and Lightweight Logs, Metrics and Traces processor for Linux, BSD, OSX and Windows
- [glfw/glfw](../repos/glfw/glfw.md) - A multi-platform library for OpenGL, OpenGL ES, Vulkan, window and input
- [mcu-tools/mcuboot](../repos/mcu-tools/mcuboot.md) - Secure boot for 32-bit Microcontrollers!
- [pret/pokeemerald](../repos/pret/pokeemerald.md) - Decompilation of Pokémon Emerald
- [linux-rdma/rdma-core](../repos/linux-rdma/rdma-core.md) - RDMA core userspace libraries and daemons
- [Cyan4973/xxHash](../repos/Cyan4973/xxHash.md) - Extremely fast non-cryptographic hash algorithm
- [kingToolbox/WindTerm](../repos/kingToolbox/WindTerm.md) - A professional cross-platform SSH&#x2F;Sftp&#x2F;Shell&#x2F;Telnet&#x2F;Tmux&#x2F;Serial terminal.
- [iOfficeAI/OfficeCLI](../repos/iOfficeAI/OfficeCLI.md) - OfficeCLI is the first and best Office suite  purpose-built for AI agents to read, edit, and auto...

---

*Last updated: 2026-09-04T21:32:08.547Z*